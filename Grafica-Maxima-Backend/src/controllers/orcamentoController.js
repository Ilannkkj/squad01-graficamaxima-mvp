...
const criarOrcamento = async (req, res) => {
    ...
    try {
        ...
        // ==========================================
        // 🛠️ PASSO 4: Vincular Acabamentos
        // ==========================================
...
const obterVendasMensais = async (req, res) => {
    try {
        const sql = `
            ...
            FROM orcamento 
            WHERE LOWER(status) IN ('aprovado', 'finalizado', 'aberto') 
        ...
        rows.forEach(row => {
            if (row.mes >= 1 && row.mes <= 12) {
                vendasPorMes[row.mes - 1] = parseFloat(row.total) || 0;
            }
        });

        return res.status(200).json(vendasPorMes);
    } catch (error) {
        console.error("Erro ao buscar vendas mensais:", error.message);
